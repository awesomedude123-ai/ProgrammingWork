...
/**
 * Instructions
 * This is a simple view being used to create, modify, delete, view the skills. While the real implementation may be different slightly,
 * this is the general idea of how it works.
 * To use the add button, you need to enter in values for each criterion. If the skill is not present, then the skill will be added.
 * If it is, the new skill will not be added.
 * To use the delete button, simply enter the name of the skill you wish to delete. If it is present, then it will delete it from the list.
 * To use the modify button, simply enter the name of the skill you wish to modify and enter in one of the criteria. If skill is present, then change will be made
 * i.e. Skill Computer Science Description New description
 ...
 * i.e. Adding a new skill will display the new skill, deleting the skill will display the last skill in the list, updating will display the modified skill, etc
 *
 *
 */
public class SkillController implements SkillManager{
    ...
    public SkillController(SkillView view, MajorController majorController, InterestController interestController, SkillTableController skillTableController){
       ...
       /** this.view.form.getListOfMajors().setSelectionMode(ListSelectionModel.MULTIPLE_INTERVAL_SELECTION);
        ...
        this.view.form.getAddButton().addActionListener(x -> {
            ...
            String expertName = this.view.form.getExpertNameValue().getText();
            if(name.isEmpty() || description.isEmpty() | majors.isEmpty() | interests.isEmpty() | waysToLearn.isEmpty() | expertName.isEmpty()) {
            ...
            this.view.form.getExpertNameValue().setText("");
        ...
        this.view.form.getQuitButton().addActionListener(x -> {
            this.skillTableController.skillPersistenceController.setSkills(this.skills);
            this.skillTableController.skillPersistenceController.writeSkillsFile();
            System.exit(0);
        });
        ...
        this.view.form.getUpdateButton().addActionListener(x -> {
            ...
            if(name.isEmpty()) {
                ...
                if(index==-1){
                    ...
                    if(!this.view.form.getExpertNameValue().getText().isEmpty()){
                        value = this.view.form.getExpertNameValue().getText();
                        isWorking = true;param = "Expert Name";
    ...
    public SkillController(SkillView view, MajorController majorController, InterestController interestController){
        this.view = view;
        skills = new ArrayList<Skill>();
        this.majorController = majorController;
        this.interestController = interestController;
    ...
    public ArrayList<Skill> filter(SkillsFilter filter, String filterText) {
        ...
        switch (filter){
            ...
            case EXPERTNAME -> {
                for(Skill skill : skills){
                    if(skill.getExpertName().equalsIgnoreCase(filterText)){
                        li.add(skill);
                    }//end if statement
                }//end for loop
            }//end case EXPERTNAME
